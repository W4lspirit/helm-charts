...
**Important**
Since Version `v1.0.0` the support for Helm v2 is dropped in favor of to support also the latest postgres Helm Chart Version which requires Helm v3 only.
...
By default, Keycloak-X is served under context `/`.
This can be changed to another context path like `/auth` as follows:
...
Keycloak-X can expose metrics via `/metrics`.
In order to achieve this, the environment variable `KC_METRICS_ENABLED` must be set.
